...
namespace HqDownloadManager.Core.Sources {
    public class UnionMangasSource : HqSource {
        public UnionMangasSource(HtmlSourceHelper htmlHelper, BrowserHelper browserHelper) : base(htmlHelper, browserHelper) {
        }
        ...
        public override async Task<Chapter> GetChapterInfo(string link) {
            try {
                ...
                var leitor = await HtmlHelper.GetSourceCodeFromUrl(link);
        ...
        public override Task<List<Hq>> GetUpdates(string updatePage) {
